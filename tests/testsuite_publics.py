...
class TestSuite_isFormatCorrect(SuperTestClass):
    ...
    def test_trueForValidJSONFile(self):
        self.assertTrue(expr = is_format_correct(file_path= pathToValidJSON), msg ="isFormatCorrect says format is not correct even though it is")

    def test_falseForInvalidJSONFile(self):
        self.assertFalse(expr = is_format_correct(file_path= pathToInvalidJSON), msg ="isFormatCorrect says format is correct even though it is not")
...
class TestSuite_getProperty(SuperTestClass):
    ...
    def test_returnsSimpleJSONPropertiesCorrectly(self, key: str):
        self.assertTrue(expr =validJSONData[key] == get_property(file_path= pathToValidJSON, keys = (key,)), msg ="getProperty return wrong value")

    def test_returnsJSONArrayCorrectly(self):
        self.assertTrue(expr =validJSONData[arrayKey] == get_property(file_path= pathToValidJSON, keys = (arrayKey,)), msg ="getProperty return wrong value")
...
class TestSuite_containsProperty(SuperTestClass):
    ...
    def test_falseIfKeysEmpty(self):
        self.assertFalse(expr = contains_property(file_path= pathToValidJSON, keys = ()), msg ="containsProperty returned True for a empty key set")

    def test_falseIfPropertyDoesntExist(self):
        self.assertFalse(expr = contains_property(file_path= pathToValidJSON, keys = invalidKeys), msg ="containsProperty returned True for a property that doesn't exist")

    def test_falseIfKeysPointToObject(self):
        self.assertFalse(expr = contains_property(file_path= pathToValidJSON, keys = (objectKey,)), msg ="key set points to json object, but containsProeprty returned True")

    @parameterized.expand(simpleTypeKeys)
    def test_trueIfPropertyExists(self, key: str):
        self.assertTrue(expr = contains_property(file_path= pathToValidJSON, keys = (key,)), msg =f"containsPropety says that property '{(key,)}' doesn't exist but it does")
...
class TestSuite_getObject(SuperTestClass):
    ...
    def test_returnsJSONObjectCorrectly(self):
        self.assertTrue(expr =validJSONData[objectKey] == get_object(file_path= pathToValidJSON, keys = (objectKey,)), msg ="getObject return wrong value")

class TestSuite_setObject(SuperTestClass):

    def test_raisesErrorIfPathInvalid(self):
        try:
            setObject(file_path= pathToNoJSON, keys = (), new_object= None)
            self.fail(msg = "this should have raised a FileNotFoundError")
        except FileNotFoundError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfInvalidJSONInFile(self):
        try:
            setObject(file_path= pathToInvalidJSON, keys = (), new_object= None)
            self.fail(msg = "this should have raised a JSONDecodeError")
        except JSONDecodeError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfNotAObject(self, key: str):
        try:
            setObject(file_path= pathToValidJSON, keys = (key,), new_object= None)
            self.fail(msg = "this should have raised a NotAObjectError")
        except NotAObjectError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfAKeyDoesNotExist(self):
        try:
            setObject(file_path= pathToValidJSON, keys = invalidKeys, new_object= None)
            self.fail(msg = "this should have raised a JSONKeyNotFoundError")
        except JSONKeyNotFoundError as ex:
            pass
        except Exception as ex:
...
class TestSuite_addObject(SuperTestClass):
    ...
    def test_raisesErrorIfPathInvalid(self):
        try:
            add_object(file_path= pathToNoJSON, keys = (), new_key="", new_object= None)
            self.fail(msg = "this should have raised a FileNotFoundError")
        except FileNotFoundError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfInvalidJSONInFile(self):
        try:
            add_object(file_path= pathToInvalidJSON, keys = (), new_key="", new_object= None)
            self.fail(msg = "this should have raised a JSONDecodeError")
        except JSONDecodeError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfNotAObject(self, object: any):
        try:
            add_object(file_path= pathToValidJSON, keys = (), new_key="", new_object= object)
            self.fail(msg = "this should have raised a NotAObjectError")
        except NotAObjectError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfAKeyDoesNotExist(self):
        try:
            add_object(file_path= pathToValidJSON, keys = invalidKeys, new_key="", new_object= None)
            self.fail(msg = "this should have raised a JSONKeyNotFoundError")
        except JSONKeyNotFoundError as ex:
            pass
        except Exception as ex:
    ...
    def test_raisesErrorIfNewKeyAlreadyExists(self):
        try:
            add_object(file_path= pathToValidJSON, keys = (), new_key= objectKey, new_object= None)
            self.fail(msg = "this should have raised a JSONKeyAlreadyExists")
        except JSONKeyAlreadyExists as ex:
            pass
        except Exception as ex:
...
class TestSuite_containsObject(SuperTestClass):
    ...
    def test_trueIfKeysEmpty(self):
        self.assertTrue(expr = contains_object(file_path= pathToValidJSON, keys = ()), msg ="containsObject returned False for a empty key set")

    def test_falseIfObjectDoesntExist(self):
        self.assertFalse(expr = contains_object(file_path= pathToValidJSON, keys = invalidKeys), msg ="containsObject returned True for a object that doesn't exist")

    @parameterized.expand(simpleTypeKeys)
    def test_falseIfKeysPointToProperty(self, key: str):
        self.assertFalse(expr = contains_object(file_path= pathToValidJSON, keys = (key,)), msg ="key set points to json property, but containsObject returned True")

    def test_trueIfObjectExists(self):
        self.assertTrue(expr = contains_object(file_path= pathToValidJSON, keys = (objectKey,)), msg =f"containsObject says that object '{(objectKey,)}' doesn't exist but it does")
